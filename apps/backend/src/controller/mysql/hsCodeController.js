...
export const getHSCodes = async (req, res) => {
  try {
    ...
    console.log(`Fetching HS codes for environment: ${environment}, forceRefresh: ${forceRefresh}`);
    
    const startTime = Date.now();
    const hsCodes = await hsCodeCacheService.getHSCodes(environment, token, forceRefresh === 'true');
...
export const searchHSCodes = async (req, res) => {
  try {
    ...
    const startTime = Date.now();
    const results = hsCodeCacheService.searchHSCodes(searchTerm, parseInt(limit));
    const duration = Date.now() - startTime;
...
export const refreshCache = async (req, res) => {
  try {
    ...
    const startTime = Date.now();
    const hsCodes = await hsCodeCacheService.getHSCodes(environment, token, true);
    const duration = Date.now() - startTime;
