...
class HSCodeCacheService {
  ...
  async getHSCodes(environment = 'sandbox', token = null, forceRefresh = false) {
  ...
  async fetchHSCodes(environment, token) {
    ...
    try {
      ...
      console.log(`HS codes fetched successfully in ${duration}ms:`, data.length);
  ...
  getCacheStatus() {
    return {
      ...
      timestamp: this.cache.timestamp ? new Date(this.cache.timestamp).toLocaleString() : null,
