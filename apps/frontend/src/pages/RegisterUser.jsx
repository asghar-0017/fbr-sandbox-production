...
import { checkRegistrationStatus, validateRegistrationStatus, getRegistrationType } from "../API/FBRService";
...
const RegisterUser = () => {
  ...
  const [fetchingRegistrationType, setFetchingRegistrationType] = useState(false);
  ...
  const handleNTNCNICChange = async (e) => {
    ...
    if (value.length >= 13 && !fetchingRegistrationType) {
      ...
      try {
        ...
        if (result.success) {
          setForm(prev => ({ ...prev, buyerRegistrationType: result.registrationType }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!form.buyerNTNCNIC || !form.buyerRegistrationType) {
        throw new Error('NTN/CNIC and Registration Type are required for verification.');
      }
      ...
      const validation = validateRegistrationStatus(form.buyerRegistrationType, apiResponse);
