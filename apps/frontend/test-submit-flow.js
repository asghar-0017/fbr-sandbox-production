...
const testSubmitFlow = async () => {
  ...
  try {
    ...
    console.log('\n📋 Step 2: Backend API Call');
    console.log('   - Endpoint: POST http://localhost:5150/api/tenant/{tenant_id}/invoices');
    console.log('   - Status:', mockBackendResponse.status);
    console.log('   - Invoice ID:', mockBackendResponse.data.data.invoice_id);
    console.log('   - FBR Invoice Number:', mockBackendResponse.data.data.fbr_invoice_number);
    ...
    console.log('   ✅ FBR Invoice Number:', mockFBRResponse.data.invoiceNumber);
  ...
  const errorScenarios = [
    {
      ...
      expectedMessage: 'Unable to connect to server. Please check your internet connection.'
    },
    {
    ...
    },
    {
      ...
      error: { response: { status: 409, data: { message: 'Invoice already exists' } } },
