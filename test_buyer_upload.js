...
async function testCheckExistingBuyers() {
  try {
    ...
    const response = await axios.post(
      `http://localhost:5150/api/tenant/${tenantId}/buyers/check-existing`,
      { buyers: testBuyers },
      {
        headers: {
    ...
    console.log(`Summary: ${summary.total} total, ${summary.existing} existing, ${summary.new} new`);
    
    if (existing.length > 0) {
      console.log('Existing buyers:');
      existing.forEach(item => {
        console.log(`  Row ${item.row}: ${item.buyerData.buyerNTNCNIC} - ${item.buyerData.buyerBusinessName} (Already exists as: ${item.existingBuyer.buyerBusinessName})`);
      });
    }
    ...
    if (newBuyers.length > 0) {
      ...
      newBuyers.forEach(item => {
        console.log(`  Row ${item.row}: ${item.buyerData.buyerNTNCNIC} - ${item.buyerData.buyerBusinessName}`);
      });
    }
    
  } catch (error) {
    console.error('Check existing failed:', error.response?.data || error.message);
  }
}
...
async function testBuyerUpload() {
  try {
    ...
    const response = await axios.post(
      `http://localhost:5150/api/tenant/${tenantId}/buyers/bulk`,
      { buyers: testBuyers },
      {
        headers: {
          'Authorization': `Bearer ${token}`,
